...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < poses.size(); i++)
    {
        ...
        // servo_pose->pose.clear(); 
        // 使用CSV文件中的数据
        for (int j = 0; j < 6; j++) {
            servo_pose->pose.push_back(poses[i][j]);
        }
        cout<<"servo_pose->pose: [" 
            << servo_pose->pose[0] << ", " << servo_pose->pose[1] << ", " << servo_pose->pose[2] << ", "
            << servo_pose->pose[3] << ", " << servo_pose->pose[4] << ", " << servo_pose->pose[5] << "]" << endl;
